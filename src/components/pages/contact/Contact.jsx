...
const Contact = () => {
  const [loading, setLoading] = useState(false); // Add state for loading indicator
  const { hasCopied, onCopy } = useClipboard('Contact@belprofmaroc.com');
  const toast = useToast(); // Move useToast hook outside the function component
  const [isSubmitDisabled, setIsSubmitDisabled] = useState(true);
  const [msg, setMsg] = useState("Votre message a été envoyé avec succès.");

   // Step 1: Set up state variables to hold the form data
  ...
  // Step 2: Create event handlers to update the state
  ...
  const handleSubmit = () => {
    ...
    axios
      .post(`${baseURL}/contact/submitContactForm`, formData)
      .then((response) => {
        console.log(response.data); // Response from the backend
        // Add any further actions after successful form submission
        setFormData({
          name: '',
          email: '',
          message: '',
        });
        setLoading(false); // Hide loading indicator after successful upload
